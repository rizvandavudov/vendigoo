...
fun GivenGoodsScreen(
    ...
    val transactions by viewModel.getTransactions(supplierId, "GIVEN_GOODS").collectAsState(emptyList())
    ...
    Scaffold(
        ...
        if (showAddDialog) {
            AmountDialog(
                title = "Verilən Mal Əlavə Et",
                onDismiss = { showAddDialog = false },
                onConfirm = { amount ->
                    viewModel.addTransaction(supplierId, amount, "GIVEN_GOODS")
                    showAddDialog = false
                }
            )
        }
